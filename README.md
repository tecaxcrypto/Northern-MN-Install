...
* First, we will do the initial collateral TX and send exactly 10,000 (ten thousand) TECAX to one of your addresses. To keep things sorted in case we setup more masternodes we will label the addresses we use.
  ...
  - Once all addresses are created send 10,000 TECAX each to them. Ensure that you send exactly 10,000 TECAX and do it in a single transaction. You can double check where the coins are coming from by checking it via coin control usually, that's not an issue.
...
```bash
masternodename ipaddress:5252 genkey collateralTxID outputID
```

An example would be

```
mn1 127.0.0.2:5252 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
```
